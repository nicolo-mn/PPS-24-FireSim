...
case class Matrix(cells: Vector[Vector[Cell]]) {
  ...
  val cols: Int = if (cells.isEmpty) 0 else cells(0).length
     ...
     * @return The cell at the specified position (r,c)
     */
   ...
   * @param r Row index
   * @param c Column index
   * @param newCell The new cell to place at (r, c)
   * @return A new Matrix with the updated cell
   */
   ...
   * @param r Row index
   * @param c Column index
   * @return True if the indices (r, c) are within the bounds of the matrix
   */
