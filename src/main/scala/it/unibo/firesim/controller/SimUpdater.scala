...
/**
 * SimUpdater is a standalone scheduler that periodically invokes a user-defined update callback.
 * It is independent of any Model or View and only manages the timing of updates.
 *
...
class SimUpdater(tickMs: Long = 100) extends Updater with Runnable {
  ...
  /**
   * Starts (or restarts) the periodic execution of the update callback.
   * Has no effect if already running.
   * If previously stopped, a fresh executor is created.
   */
  ...
  /**
   * Stops the scheduler and prevents further callbacks.
  ...
  /**
   * The Runnable run method invoked by the ScheduledExecutorService.
   * It checks the running and paused flags and executes the user-provided callback.
   */
