...
/** Updater is the interface for components responsible for managing the
  * simulation's ticking mechanism. It defines the basic control operations for
  * the simulation lifecycle, such as starting, stopping, and pausing/resuming
  * periodic updates
  */
trait Updater:
  ...
  /** Checks whether the updater is currently running.
    * @return
    *   true if the simulation is active, false if simulation is stopped.
    */
  def isRunning: Boolean

  /** Checks whether the updater is currently paused.
    * @return
    *   true if simulation is not paused.
    */
