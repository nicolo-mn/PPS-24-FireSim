...
/**
 * SimController coordinates the interactions between the simulation model, the updater (tick scheduler),
 * and the view. It safely updates the model parameters and processes user commands via messages.
 *
 * @param model The simulation model.
 * @param updater The simulation updater handling the ticks.
 */
class SimController(
    model: MockSimModel,
    updater: SimUpdater
   ...
   * @param setter The model setter function.
  ...
  /**
   * Sets the view and the callback to be called by the updater on each simulation tick.
   */
  def setView(): Unit =
